  ...
  def export_all
    if params.include?(:export) && !params[:export][:date].blank?
      ...
      if week == 1
        ...
        appointments = Appointment.where("date BETWEEN ? AND ?", week_start.beginning_of_day, week_end.end_of_day).all.order('date ASC') #obtengo todos los turnos de la semana
        #genero un hash con dia => arreglo vacio
        week = AppointmentsUtils.get_hash_week(week_start,week_end)
      else
        appointments = Appointment.where("date BETWEEN ? AND ?", datetime.beginning_of_day, datetime.end_of_day).all.order('date ASC') #obtengo todos los turnos del dia
