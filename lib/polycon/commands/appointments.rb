      ...
      class Create < Dry::CLI::Command
        ...
        def call(date:, professional: "", name: "", surname: "", phone: "", notes: nil)
          ...
          if ! Utils.valid_date_with_hour?(date) #reviso que la fecha ingresada sea correcta
            ...
            validation_result.each{ |key,value| warn "El parametro #{key} no puede estar vacio" }
            warn "ERROR: Los datos recibidos para el turno no son correctos\nEjemplo de uso: ''2021-09-16_13-00' --professional='Alma Estevez' --name=Carlos --surname=Carlosi --phone=2213334567'"
            ...
            puts (if appointment.save_file #Guardo la informacion del turno
              then "Se han guardado los datos del turno del dia #{date} para el profesional #{professional}"#Mensaje de exito
              else "Ocurrió un error al guardar los datos del turno del dia #{date} para el profesional #{professional}, por favor intentelo de nuevo" #Mensaje de error
              end)
          end
        end
      end
      ...
      class Cancel < Dry::CLI::Command
        ...
        def call(date:, professional: "")
          ...
          if Utils.blank_string?(professional) || ! Models::Professionals.exist?(professional) #reviso si existe el profesional
            ...
            appointment = Models::Appointments.get_appointment(date,professional)
            puts (if appointment.cancel #elimino el turno
              then "Se ha cancelado el turno del dia #{date} del profesional #{professional}"#Mensaje de exito
              else "No se ha podido cancelar el turno del profesional #{professional} del dia #{date}" #Mensaje de error
              end)
          end
        end
      end
      ...
      class CancelAll < Dry::CLI::Command
        ...
        def call(professional:,**)
          ...
          if Utils.blank_string?(professional)
            ...
            puts (if profesional.cancel_appointments #elimino los turnos del profesional
              then "Se ha completado el cancelado de todos los turnos del profesional #{professional}" #Mensaje de exito
              else "No se pudo completar el cancelado de todos los turnos del profesional #{professional}" #Mensaje de error
              end)
          end
        end
      end
      ...
      class Reschedule < Dry::CLI::Command
        ...
        def call(old_date:, new_date:, professional: "")
          ...
          if ! Utils.valid_date_with_hour?(new_date) #reviso si la fecha recibida es un formato valido
            ...
            puts (if appointment.rename(new_date) #renombro el turno
              then "Se ha reagendado el turno del profesional #{professional} del dia #{old_date} al dia #{new_date}" #Mensaje de exito
              else "No se ha podido reagendar el turno del dia #{old_date} al dia #{new_date}" #Mensaje de error
              end)
          end
        end
      end
      ...
      class Edit < Dry::CLI::Command
        ...
        def call(date:, professional: "", **options)
          ...
          if ! check_result.empty?
            check_result.each{ |key,value| warn "El parametro #{key} no puede ser una cadena vacia" }
            warn "ERROR: Las opciones para editar no son validas"
          elsif Utils.blank_string?(professional) || ! Models::Professionals.exist?(professional) #reviso si existe el profesional
            warn "ERROR: El profesional #{professional} no existe en el sistema o no fue especificado"
          elsif Utils.blank_string?(date) || ! Models::Appointments.exist?(date,professional) #reviso que si existe el turno para el profesional
            warn "ERROR: El turno del dia #{date} del profesional #{professional} no existe en el sistema"
            ...
            appointment = Models::Appointments.get_appointment(date,professional)
            puts (if appointment.edit_file(options) #Edita los datos del turno
              then "Se han editado los datos del turno del dia #{date} para el profesional #{professional}" #Mensaje de exito
              else "Ocurrió un error al editar los datos del turno del dia #{date} para el profesional #{professional}, por favor intentelo de nuevo" #Mensaje de error
              end)
          end
        end
      end
